...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({
  ...
  const handleDownloadReport = () => {
    ...
    a.href = url;
    const reportName = 'refactor_report.json';
  ...
  const handleDownloadCode = () => {
    if (!refactoredCode || !selectedFile) return;
    const blob = new Blob([refactoredCode], { type: 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `refactored_${selectedFile.replace(/[\/\\]/g, '_')}`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
              ...
              {status === 'refactored' && (
                <>
                  {files && files.length > 1 && (
                    <div className="my-6">
                      <label htmlFor="file-selector" className="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-2">
                  ...
                  {selectedFile && originalCode !== undefined && refactoredCode !== undefined && (
                    <CodeComparer
