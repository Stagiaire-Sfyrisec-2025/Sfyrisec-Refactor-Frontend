...
const HistoryDetailModal: React.FC<HistoryDetailModalProps> = ({ isOpen, onClose, project }) => {
  ...
  const improvement = initialAnalysis?.cyclomaticComplexity > 0
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-50 dark:bg-dark-main-bg p-6 rounded-lg">
            ...
            <div className="space-y-3 text-sm">
              ...
              <p><strong>Lignes de code:</strong> {initialAnalysis?.linesOfCode ?? 'N/A'}</p>
              <p><strong>Maintenabilité:</strong> {initialAnalysis?.maintainability ? initialAnalysis.maintainability.toFixed(2) : 'N/A'}</p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 dark:bg-dark-main-bg p-6 rounded-lg">
            ...
            <div className="space-y-3 text-sm">
              ...
              <p><strong>Lignes de code:</strong> {refactoredAnalysis?.linesOfCode ?? 'N/A'}</p>
              <p><strong>Maintenabilité:</strong> {refactoredAnalysis?.maintainability ? refactoredAnalysis.maintainability.toFixed(2) : 'N/A'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-8 pb-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <div className="bg-blue-100 dark:bg-blue-900/50 p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-blue-600 dark:text-blue-300">{project.fileCount ?? 'N/A'}</p>
                </div>
                 <div className="bg-purple-100 dark:bg-purple-900/50 p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-purple-600 dark:text-purple-300">{refactoredAnalysis?.maintainability ? refactoredAnalysis.maintainability.toFixed(2) : 'N/A'}</p>
                </div>
            </div>
        </div>
      </div>
